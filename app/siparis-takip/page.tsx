    ...
    <div className="bg-white min-h-screen">
      <div className="max-w-4xl mx-auto px-3 sm:px-6 lg:px-8 py-6 sm:py-10 space-y-6 sm:space-y-8 pb-24 lg:pb-12">
        <div className="text-center space-y-2">
          <div className="inline-flex p-3 bg-red-50 text-[#e60012] rounded-2xl mb-1 shadow-2xs">
          ...
          <form onSubmit={handleSearch} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full text-base sm:text-xs p-3 bg-stone-50 border border-stone-300 rounded-xl focus:bg-white focus:outline-none focus:border-[#e60012] font-mono text-stone-900 transition"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full text-base sm:text-xs p-3 bg-stone-50 border border-stone-300 rounded-xl focus:bg-white focus:outline-none focus:border-[#e60012] text-stone-900 transition"
              />
            </div>
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full py-3.5 bg-[#e60012] hover:bg-[#c90010] active:scale-95 text-white text-xs font-bold rounded-xl shadow-md transition flex items-center justify-center gap-2 min-h-[48px] disabled:opacity-50"
            >
              ...
              <div className="bg-white rounded-3xl border border-stone-200 p-5 sm:p-10 shadow-2xs space-y-6 sm:space-y-8 animate-slide-up">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 sm:pb-6 border-b border-stone-100">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="px-3 py-1 bg-red-50 text-[#e60012] text-xs font-bold rounded-full capitalize border border-red-200">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center text-xs">
                  <div className="p-3 rounded-2xl border border-stone-200/80 bg-stone-50/50 space-y-1">
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs mx-auto shadow-2xs ${
                        currentStep >= 0 ? 'bg-[#e60012] text-white' : 'bg-stone-200 text-stone-500'
                      }`}
                    >
                      {currentStep > 0 ? '✓' : '1'}
                    </div>
                  ...
                  <div className="p-3 rounded-2xl border border-stone-200/80 bg-stone-50/50 space-y-1">
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs mx-auto shadow-2xs ${
                        currentStep >= 1 ? 'bg-[#e60012] text-white animate-pulse' : 'bg-stone-200 text-stone-500'
                      }`}
                    >
                      {currentStep > 1 ? '✓' : '2'}
                    </div>
                    <div className="font-bold text-[#e60012] text-xs">Hazırlanıyor</div>
                    <div className="text-[10px] text-stone-400">Depomuzda</div>
                  </div>

                  <div className="p-3 rounded-2xl border border-stone-200/80 bg-stone-50/50 space-y-1">
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs mx-auto shadow-2xs ${
                        currentStep >= 2 ? 'bg-[#e60012] text-white' : 'bg-stone-200 text-stone-500'
                      }`}
                    >
                      {currentStep > 2 ? '✓' : '3'}
                    </div>
                ...
                {order.tracking_number && (
                  <div className="p-4 bg-red-50/40 border border-red-200 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                      <Truck className="w-6 h-6 text-[#e60012] shrink-0" />
                      <div>
                    ...
                    <a
                      ...
                      className="inline-flex items-center justify-center gap-1.5 px-4 py-2 bg-[#e60012] hover:bg-[#c90010] text-white font-bold text-xs rounded-xl shadow-xs transition"
                    >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-stone-100 text-xs">
                  <div className="space-y-3">
                    ...
                    {order.has_gift_wrap && (
                      <div className="p-3.5 rounded-xl bg-stone-50 border border-stone-200 text-stone-800 space-y-1">
                        <div className="flex items-center gap-1.5 font-bold text-[11px] text-[#e60012]">
                          <Gift className="w-3.5 h-3.5" />
                          <span>Özel Hediye Paketi Talebi Mevcut</span>
                        </div>
                        {order.gift_note && (
                          <p className="text-[11px] italic text-stone-600">Not: "{order.gift_note}"</p>
                        )}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="divide-y divide-stone-100 bg-stone-50 rounded-xl p-3">
                      ...
                      <div className="pt-2 flex justify-between font-bold text-stone-900">
                        ...
                        <span className="text-[#e60012]">{formatPrice(order.total_amount)}</span>
                      </div>
                    </div>
                  </div>
                </div>
